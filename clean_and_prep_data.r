...
, stringsAsFactors = F
  ...
  mutate(
    assessment_title = gsub(pattern = "MPD-H"
    , replacement = "h"
    , x = assessment_title
    )
  ) %>%
  mutate(
    assessment_title = gsub(
      pattern = "[-()\\s]"
    , replacement = ""
    , x = assessment_title
    )
...
wide_data_2 <- wide_data %>%
  ...
  , content_progress_count = ifelse(
      is.na(content_progress_count)
    , 0
    , content_progress_count
    )
  , people_talked_to = as.numeric(people_talked_to)
  , NEW_MONTHLY_A = ifelse(
      is.na(as.numeric(NEW_MONTHLY_A))
    , 0
