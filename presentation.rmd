...
params:
  timeint: month
  usersetcsvdir: user_sets
  usersetqdir: user_set_queries
...
back_env$param_time_interval <- timeint_chr
...
time_to_threshold_hist_data <- back_env$get_time_to_threshold_hist_data(
  back_env$wide_data
  , back_env$param_efficiency_analysis_threshold_pct
  , timeframe = back_env$param_efficiency_analysis_threshold_week
)
...
pct_past_timeframe_data  <- time_to_threshold_hist_data %>%
  ...
  summarise(
    pct_past_timeframe = mean(weeks_to_threshold > (back_env$param_efficiency_analysis_threshold_week - 1))
  )
...
avg_time_to_thresh_gloo <- time_to_threshold_hist_data %>%
  filter(
    gloo_cohort
    , weeks_to_threshold <= (back_env$param_efficiency_analysis_threshold_week - 1)
  ) %>%
...
of their support goal. Among Gloo users who reached the `r round(100 * back_env$param_efficiency_analysis_threshold_pct, 1)`%
